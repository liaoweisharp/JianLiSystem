(function () {
    var baseData = {};
    function RYZC_C(projectId, type) {
        this.projectId = projectId;
        initDom(projectId, type);

    }
    
    RYZC_C.Prefix = "RYZC_C_";

    function initDom(projectId, type) {
        $("#" + RYZC_C.Prefix + projectId).html(loading);
        if (type == "update") {
            $("#" + RYZC_C.Prefix + "_Add").show();
        }
        $invokeWebService_2("~WebService_XiangMu.getCurrentMonthRenYuan", { projectId: projectId }, null, successCallBack, errorCallBack, null, { userContent: "getCurrentMonthRenYuan", projectId: projectId, type: type });
    }
    function successCallBack(result, context) {
        if (context.userContent == "getCurrentMonthRenYuan") {
            var projectId = context.projectId;
            var type = context.type
            
            var data = result;
            if (data.length == 0) {
                $("#" + RYZC_C.Prefix + projectId).html(noResult);
            }
            else {
                $("#" + RYZC_C.Prefix + projectId).html(html_ShowTable(data, projectId, type));
    ...
    RYZC_C.clickDetail = function (id) {
