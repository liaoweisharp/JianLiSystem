...
(function () {
    ...
    function initDom(userId, type) {
        $("#" + GZJL.Prefix + userId).html(loading);
        $invokeWebService_2("~WebService_RenLi.getGongZuoJingLiByUserId", { userId: userId }, null, successCallBack, errorCallBack, null, { userContent: "getXueXiJingLiByUserId", userId: userId, type: type });
    }
    function successCallBack(result, context) {
        if (context.userContent == "getXueXiJingLiByUserId") {
            ...
            var data = result;
            if (data.length == 0) {
                $("#" + GZJL.Prefix + userId).html(noResult);
            }
            else {
                baseData["工作经历"] = data;

                $("#" + GZJL.Prefix + userId).html(html_ShowTable(data, userId, type));
    ...
    GZJL.clickEdit = function (id) {

        var jsonArray = createJson();
        var option = { type: "update", data: {} };
        //jBox options
        var optionJbox = { title: "编辑", width: 850, buttons: { "更新": "1", "取消": "0" }, showClose: false, submit: GZJL._clickEdit };
        var obj = baseData["工作经历"].firstOrDefault("rl_jl_Id", id);
        if (obj) {
            var bindObj = new bind(jsonArray, obj, option, optionJbox);
        }
    }
    GZJL.clickDel = function (id, userId) {
        var ramdomId = String.randomString(6);
        $.jBox.confirm(String.format("<input id='{0}' type='hidden'>你确定要删除这条记录吗？", ramdomId), "确定删除吗？", GZJL._clickDel, { buttons: { "删除": "1", "取消": "0"} })
        $("#" + ramdomId).data("data", { id: id, userId: userId });
    }
    GZJL._clickAdd = function (v, h, f) {
        if (v == "1") {
    ...
    GZJL._clickEdit = function (v, h, f) {
        if (v == "1") {
            var bindObj = h.find("[name='" + bind.Obj + "']").data("data");
            var jsonArray = bindObj.ShouJiData();
    ...
    function createJson() {
        var jsonArray = [];
        jsonArray.push({ itemId: "rl_jl_JieShao", type: "text", title: "单位名称"});
    ...
    function html_ShowTable(datas, userId, type) {
        ...
        if (datas.length > 0) {
            ...
            for (var j = 0; j < datas.length; j++) {
                ...
                if (!type || type == "update") {
                    str.push(String.format("<td class='tdOpation'><span class='opation'><a href='javascript:void(0);' onclick=\"GZJL.clickDetail('{0}')\">详细</a>|<a href='javascript:void(0);' onclick=\"GZJL.clickEdit('{0}')\">编辑</a>|<a href='javascript:void(0);' onclick=\"GZJL.clickDel('{0}','{1}')\">删除</a></span></td>", id, userId));
                }
                else if (type == "review") {
                    str.push(String.format("<td class='tdOpation'><span class='opation'><a href='javascript:void(0);' onclick=\"GZJL.clickDetail('{0}')\">详细</a></span></td>", id, userId));
                }
                str.push("</tr>");
            }
            str.push("</table>");
